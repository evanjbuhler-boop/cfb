...
export async function seedDatabase(): Promise<{ teams: number; players: number; recruits: number }> {
  const existing = await prisma.team.count();
  if (existing >= 10) return { teams: existing, players: 0, recruits: 0 };
  ...
  return { teams: TEAMS_DATA.length, players: allPlayers.length, recruits: 600 };
}
