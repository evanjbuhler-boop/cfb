...
const DEV_TRAITS = ["NORMAL","NORMAL","NORMAL","NORMAL","IMPACT","IMPACT","STAR","ELITE"];
const PLAYER_TRAITS = ["COACHES_FAVORITE","CLUTCH_GENE","FILM_ROOM_RAT","MOTOR","SHOWBOAT","LOCKER_ROOM_LEADER","RAW","WORKHORSE","COMEBACK_KID","ELUSIVE_BACK","FIELD_GENERAL","HOT_HEAD"];
...
const ROSTER_DEF = [[2,"QB"],[3,"RB"],[5,"WR"],[2,"TE"],[5,"OL"],[4,"DL"],[4,"LB"],[4,"CB"],[3,"S"]];
const YEARS = ["FR","FR","SO","SO","JR","JR","SR","SR","GR"];

function generatePlayer(position, year, teamPrestige) {
  const { first, last } = randName();
  const yearMod = { FR:0, SO:5, JR:10, SR:14, GR:16 };
...
function generateRecruit(nationalRank, enrollYear) {
  const { first, last } = randName();
...
async function seed() {
  ...
  if (existing > 0) {
    console.log(`[seed] Already seeded (${existing} teams). Skipping.`);
  ...
  await prisma.coach.createMany({ data: allCoaches });
  ...
  console.log(`[seed] Done — ${TEAMS_DATA.length} teams, ${allPlayers.length} players, 600 recruits.`);
  await prisma.$disconnect();
}
